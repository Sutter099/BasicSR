...
#     # 2. 实例化 LKLGLBlock 模块
#     # dim 必须与输入通道数一致
#     # sr_ratio > 1 会激活 LocalAgg 部分，等于1则跳过
#     lklgl_module = LGLBlock(
#         dim=channels,
#         num_heads=8,       # 注意力头数，通常能被dim整除
#         mlp_ratio=4.,
#         sr_ratio=2         # 设置大于1来测试完整的LKLGL模块
#     )
#
#     # 3. 将输入张量传入模块
#     output_tensor = lklgl_module(input_tensor)
#
#     # 4. 打印输入和输出的形状，验证其保持不变
#     print(f"输入张量形状: {input_tensor.shape}")
#     print(f"输出张量形状: {output_tensor.shape}")
#
#     # 5. 检查输入和输出形状是否一致
#     assert input_tensor.shape == output_tensor.shape
#     print("\n✅ 测试成功！LKLGLBlock 模块正常工作，且输入输出形状一致。")
