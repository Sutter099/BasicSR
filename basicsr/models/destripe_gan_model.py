...
class SGM(nn.Module):
    ...
    def forward(self, img):
        ...
        for m in range(b):
            ...
            noise_list.append(torch.clamp(imgn_m, 0., 1.))
...
class DestripeGANModel(SRModel):
    ...
    def optimize_parameters(self, current_iter):
        ...
        self.real_B_recon = net_g_bare(fake_A_norm)
        
        real_B_norm = self.gt * 2.0 - 1.0
        self.fake_B_I = net_g_bare(real_B_norm)
        ...
        l_d_real = self.cri_gan(real_d_pred[0], True, is_disc=True)
        for i in range(1, len(real_d_pred)):
            l_d_real = l_d_real + self.cri_gan(real_d_pred[i], True, is_disc=True)
        l_d_real = l_d_real / len(real_d_pred)
        loss_dict['l_d_real'] = l_d_real
        l_d_real.backward()

        # fake_B and real_B_recon are fake. Original code used pool of 50.
        fake_d_pred = self.net_d(self.output.detach())
        l_d_fake1 = self.cri_gan(fake_d_pred[0], False, is_disc=True)
        for i in range(1, len(fake_d_pred)):
            l_d_fake1 = l_d_fake1 + self.cri_gan(fake_d_pred[i], False, is_disc=True)
        l_d_fake1 = l_d_fake1 / len(fake_d_pred)
        
        recon_B_pred = self.net_d(self.real_B_recon.detach())
        l_d_fake2 = self.cri_gan(recon_B_pred[0], False, is_disc=True)
        for i in range(1, len(recon_B_pred)):
            l_d_fake2 = l_d_fake2 + self.cri_gan(recon_B_pred[i], False, is_disc=True)
        l_d_fake2 = l_d_fake2 / len(recon_B_pred)
